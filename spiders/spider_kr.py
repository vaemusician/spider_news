...
class KrSpider(Spider):
    def __init__(self, req_params):
        ...
        self.source = 'http://www.korea.kr'
        self.source_url = 'http://www.korea.kr'
    ...
    def get_news_last_list(self):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'http://www.korea.kr/news/top50List.do'
    ...
    def analyze_news_last_list(self, news_last_list_text):
        ...
        trs = text_tree.xpath('//*[@id="container"]/div/article/div[1]/div/div[1]/ul/li')
        for tr in trs:
            detail = dict()
        ...
        trs_hot = text_tree.xpath('//*[@id="container"]/div/article/div[1]/div/div[2]/ul/li')
        for tr in trs_hot:
            detail = dict()
            href_url = ''.join(tr.xpath('a/@href') or '')
            if href_url == '':
    ...
    def get_news_result_cnt(self, news_url,keyword=''):
        ...
        if article.cleaned_text:
            ...
            cont = ''.join(text_tree.xpath('//*[@id="container"]/div/article/div[2]/div[1]//text()')).replace(' ',
                                                                                                              '').replace(
                '\t', '').replace('\r\n\r\n\r\n\r\n\r\n\r\n', '')
        news_post = dict(
    ...
    def get_search_result(self):
        ...
        params = (
            ('srchFlag', 'Y'),
            ('keyword', self.req_params['keyword']),
            ('oldKeyword', self.req_params['keyword']),
            ('resultKeyword', self.req_params['keyword']),
            ('sort', 'date'),
            ('scnt', '1'),
            ('punit', '20'),
            ('where', 'news_korea_new'),
            ('dateRange', 'all'),
            ('range', 'ALL'),
            ('op', 'and'),
            ('multiFlag', 'Y'),
            ('dtl_news_type', 'pol,gov'),
            ('tabdept', '1'),
        )
        wb_url = 'http://www.korea.kr/totalSearch.do'
        proxy = {'http': self.proxy,
                 'https': self.proxy}
        wb_res = requests.get(wb_url, headers=head, proxies=proxy, params=params, verify=False)
        wb_res.encoding = 'utf-8'
        return wb_res.text
    ...
    def analyze_search_result(self, search_result_text):
        ...
        trs = text_tree.xpath("//div[@class='result_list']/ul/li/span/strong/a")
        for tr in trs:
            detail = dict()
