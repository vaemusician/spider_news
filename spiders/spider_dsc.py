...
# Y 越共电子报（越）
class DscSpider(Spider):
    def __init__(self, req_params):
        super().__init__(req_params)
        self.source = 'cn.dangcongsan.vn'
        self.source_url = 'http://cn.dangcongsan.vn/'
    ...
    def get_news_last_list(self, page=1):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'http://cn.dangcongsan.vn/focus.html?page={}'.format(str(page))
    ...
    def analyze_news_last_list(self, news_last_list_text):
        ...
        trs = text_tree.xpath('//div[@class="w-list"]')
        for tr in trs:
            detail = dict()
    ...
    def get_search_result(self):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'http://cn.dangcongsan.vn/tim-kiem.html?q={}&cid=297'.format(self.req_params['keyword'])
        proxy = {'http': self.proxy,
                 'https': self.proxy}
        wb_res = requests.get(wb_url, headers=head, proxies=proxy, verify=False)
        wb_res.encoding = 'utf-8'
    ...
    def analyze_search_result(self, search_result_text):
        ...
        trs = text_tree.xpath('//div[@class="col-md-8 col-xs-12"]/a')
        for tr in trs:
            detail = dict()
