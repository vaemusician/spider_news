...
# Y 越南通讯社（越）
class TtxvnSpider(Spider):
    def __init__(self, req_params):
        super().__init__(req_params)
        self.source = 'https://zh.vietnamplus.vn'
        self.source_url = 'https://zh.vietnamplus.vn'
    ...
    def get_news_last_list(self):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'https://zh.vietnamplus.vn/mostread.vnp'
    ...
    def analyze_news_last_list(self, news_last_list_text):
        ...
        trs = text_tree.xpath('//div[@class="clearfix"]/article')
        for tr in trs:
            detail = dict()
            href_url = ''.join(tr.xpath('a[1]/@href') or '')
            if href_url == '':
                continue
            if 'http' in href_url:
                detail['url'] = href_url
    ...
    def get_search_result(self):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'https://zh.vietnamplus.vn/timkiem/{}.vnp'.format(self.req_params['keyword'])
        proxy = {'http': self.proxy,
                 'https': self.proxy}
        wb_res = requests.get(wb_url, headers=head, proxies=proxy, verify=False)
        wb_res.encoding = 'utf-8'
    ...
    def analyze_search_result(self, search_result_text):
        ...
        trs = text_tree.xpath('//*[@id="st-container"]/div/div/div/div[2]/div[2]/div[1]/section/div/article/a[1]')
        for tr in trs:
            detail = dict()
...
#     "keyword": "2020"
# }
# spid = TtxvnSpider(req_params=req)
