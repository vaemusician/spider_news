...
# Y  台湾关怀中国人权联盟(台湾人权促进会)
class TaoSpider(Spider):
    def __init__(self, req_params):
        super().__init__(req_params)
        self.source = 'www.tachr.org'
        self.source_url = 'https://www.tahr.org.tw'
    ...
    def get_news_last_list(self, current_page=1):
        ...
        head[
            'User-Agent'] = self.ua
        wb_url = 'https://www.tahr.org.tw/info/press-release?page={}'.format(str(current_page))  # 每页10条按时间排序
    ...
    def analyze_news_last_list(self, news_last_list_text):
        ...
        trs = text_tree.xpath('//*[@id="block-system-main"]/div/div[1]/div')
        for tr in trs:
            detail = dict()
            href_url = ''.join(tr.xpath('div/div[1]/div/div/div/div/a/@href') or '')
            if href_url == '':
                continue
            if 'http' in href_url:
                detail['url'] = href_url
